...
# 1.- Creación de un vector de color
color=c("lightcyan", "darkviolet", "khaki1")
...
color=c("lightcyan", "lavender", "floralwhite")
...
HG<-ggplot(BD1, aes(x=largo_aleta_mm))+
  geom_histogram(col="darkslateblue", fill="ghostwhite")+
  ggtitle("Histograma")+
...
#si se quita el color=espece dará el gráfico con puntos negros
...
GD3<-ggplot(BD1, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  geom_point(aes(color=especie))+
  scale_color_manual(values=c('hotpink','darkblue','gold'))+
  ggtitle("Gráfico de dispersión")+
  xlab("largo del pico (mm)")+
  ylab("grosor del pico (mm)")+
  theme_light()
